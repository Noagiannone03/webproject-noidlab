...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Pour chaque section, appliquer le traitement sur l'élément .circle
  sections.forEach(section => {
    const circle = section.querySelector('.circle');
    if (!circle) return;
    
    // Récupérer l'URL de l'icône à partir de l'attribut data-icon
    const iconUrl = circle.getAttribute('data-icon');
    circle.style.backgroundImage = `url('${iconUrl}')`;
    
    // Ajoute directement la classe "active" pour déclencher l'état final
    circle.classList.add('active');
    
    // Si vous souhaitez prévoir une animation de sortie (bounce-out) quand l'élément perdrait son statut actif,
    // vous pouvez définir ici un timer ou une autre logique. Pour l'instant, on le laisse en état "active".
  });
});
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Fonction pour calculer la largeur totale d'une carte (offsetWidth + marge droite)
  function getCardWidth() {
    const firstCard = sliderContainer.querySelector('.flip-card');
    if (firstCard) return 320; // Valeur par défaut si aucune carte n'est trouvée
    const cardWidth = firstCard.offsetWidth; // La largeur réelle (ex. 310px)
    const computedStyle = window.getComputedStyle(firstCard);
    const marginRight = parseFloat(computedStyle.marginRight) || 0;
    const totalWidth = cardWidth + marginRight;
    console.log("Largeur totale d'une carte :", totalWidth);
    return totalWidth;
  }
  ...
  flipBtn.addEventListener('click', function(e) {
    e.stopPropagation();
    // Sélectionne la carte actuellement visible (ou la première carte) pour le flip
    const currentCard = sliderContainer.querySelector('.flip-card');
    if (currentCard && typeof flipCard === "function") {
      flipCard(currentCard);
    } else {
      console.error("Aucune carte trouvée ou la fonction flipCard n'est pas définie");
    }
  });
});
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (/Mobi|Android/i.test(navigator.userAgent)) {
    // Écoute globalement les touchstart sur le body
    document.body.addEventListener('touchstart', function() {
      // Si un élément est focusé et qu'il n'est pas le body lui-même, on le blur.
